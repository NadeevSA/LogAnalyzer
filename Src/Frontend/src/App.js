...
function App() {
  ...
  const [btnRepo, setBtnRepo] = useState(true)
      ...
      <Header
        leftSide={
          ...
          <HeaderModule indent="m">
            <Button style={{margin: '0 25px 0 0'}} label="Старт анализа" onClick={() => fetchCalculation()} loading={btnLoad}/>
            <Button style={{margin: '0 25px 0 0'}} view="secondary" label="Результат" onClick={() => {
              setBtnResult(!btnResult)
              setBtnRepo(!btnRepo)
              }} disabled={!btnRepo}/>
            <Button style={{margin: '0 25px 0 0'}} view="secondary" label="Репозиторий" onClick={() => {
              setBtnRepo(!btnRepo)
              setBtnResult(!btnResult)
              }} disabled={!btnResult}/>
          </HeaderModule>
